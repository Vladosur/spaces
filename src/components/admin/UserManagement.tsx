...
const UserManagement: React.FC = () => {
    ...
    const [formData, setFormData] = useState<{ name: string; email: string; role: Role }>({ name: '', email: '', role: RoleConst.USER });
    const [modalMode, setModalMode] = useState<'add' | 'edit'>('add');
    const [view, setView] = useState<'card' | 'table'>(defaultView);
    const [activeTab, setActiveTab] = useState<Role>(RoleConst.USER);
    ...
    const handleOpenModal = (mode: 'add' | 'edit', user: User | null = null) => {
        setModalMode(mode);
        setCurrentUser(user);
        const defaultRole: Role = mode === 'add' ? activeTab : (user ? user.role : RoleConst.USER);
    ...
    const handleCloseModal = () => {
        ...
        setCurrentUser(null);
        setFormData({ name: '', email: '', role: RoleConst.USER });
    };
    ...
    const RoleBadge = ({ role }: { role: Role }) => (
        <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
            ...
            {role === RoleConst.ADMIN ? 'Admin' : 'User'}
        </span>
    );

    const tabOptions = [
        { label: 'Utenti', value: RoleConst.USER },
        { label: 'Admin', value: RoleConst.ADMIN },
    ];
                            ...
                            <div key={user.id} className="bg-card border border-border rounded-xl p-4 flex flex-col gap-4 hover:shadow-md transition-all hover:-translate-y-1">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        {user.role === RoleConst.ADMIN ? <ShieldCheck className="text-blue-500" size={24} /> : <UserCircle className="text-muted-foreground" size={24}/>} 
                        ...
                        <div>
                            <label htmlFor="userRole" className="block text-sm font-medium text-muted-foreground">Ruolo</label>
                             <select 
                                ...
                                className={inputClasses}
                            >
                                        <option value={RoleConst.USER}>User</option>
                                        <option value={RoleConst.ADMIN}>Admin</option>
                            </select>
                        </div>
                    </div>
